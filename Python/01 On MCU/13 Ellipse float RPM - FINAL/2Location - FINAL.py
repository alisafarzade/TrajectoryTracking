    ...
    for pic, contour in enumerate(contours):
        ...
        if area > 50 and area<50000:
            ...
            if time.time() - last_step_time > 0.1:
                ...
                x2 = x_Center1 * cos(radians(green_angle)) + y_Center1 * sin(radians(green_angle))
                x3 = x_Center1 * sin(radians(green_angle)) - y_Center1 * cos(radians(green_angle))
                x2d = Xd * cos(thetaD) + Yd * sin(thetaD)
                x3d = Xd * sin(thetaD) - Yd * cos(thetaD)
