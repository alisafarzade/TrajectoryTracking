...
wifiServerBegin()
def serverSendData():
    global useServer, packet_green
    if useServer:
        b = bytearray()
        b.append(packet_green[0])
        b.append(packet_green[1])
        b.append(packet_green[2])
        b.append(packet_green[3])
        b.append(packet_green[4])
        b.append(packet_green[5])
        client.send(b)
    ...
    for pic, contour in enumerate(contours):
        ...
        if area > 50 and area<50000:
            ...
            if time.time() - last_step_time > 0.1:
                ...
                df2 = pd.DataFrame([[t1, x_Center1, y_Center1, green_angle]], columns=['time', 'X', 'Y', 'theta'])
                Xd = - 30 * cos(t1 * speed) + circle_x + 30
                if int((t1*speed)/(2*pi))%2 == 1:
                    Xd = 30 * cos(t1 * speed) + circle_x - 30
                Yd = 30 * sin(t1 * speed) + circle_y
                thetaD = atan2(cos(t1*speed), -sin(t1*speed))
