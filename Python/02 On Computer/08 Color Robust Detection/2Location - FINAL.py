...
# Robot 1 = Red
def xcd_1():
    return rx/100 * cos(t1*speed)
def ycd_1():
	return ry/100 * sin(t1*speed)
def thetad_1():
	theta = atan2(ry/100*cos(t1*speed), -rx/100*sin(t1*speed))
...
while 1:
    ...
    if useRobustColorDetection:
        red_histh = cv2.calcHist([hsvFrame],[0],red_mask,[179],[0,179])  
        red_hists = cv2.calcHist([hsvFrame],[1],red_mask,[255],[0,255])  
        red_histv = cv2.calcHist([hsvFrame],[2],red_mask,[255],[0,255])  

        green_histh = cv2.calcHist([hsvFrame],[0],green_mask,[179],[0,179])  
        green_hists = cv2.calcHist([hsvFrame],[1],green_mask,[255],[0,255])  
        green_histv = cv2.calcHist([hsvFrame],[2],green_mask,[255],[0,255])  

        blue_histh = cv2.calcHist([hsvFrame],[0],blue_mask,[179],[0,179])  
        blue_hists = cv2.calcHist([hsvFrame],[1],blue_mask,[255],[0,255])  
        blue_histv = cv2.calcHist([hsvFrame],[2],blue_mask,[255],[0,255])  

        yellow_histh = cv2.calcHist([hsvFrame],[0],yellow_mask,[179],[0,179])  
        yellow_hists = cv2.calcHist([hsvFrame],[1],yellow_mask,[255],[0,255])  
        yellow_histv = cv2.calcHist([hsvFrame],[2],yellow_mask,[255],[0,255])  

        orange_histh = cv2.calcHist([hsvFrame],[0],orange_mask,[179],[0,179])  
        orange_hists = cv2.calcHist([hsvFrame],[1],orange_mask,[255],[0,255])  
        orange_histv = cv2.calcHist([hsvFrame],[2],orange_mask,[255],[0,255])  

        pink_histh = cv2.calcHist([hsvFrame],[0],pink_mask,[179],[0,179])  
        pink_hists = cv2.calcHist([hsvFrame],[1],pink_mask,[255],[0,255])  
        pink_histv = cv2.calcHist([hsvFrame],[2],pink_mask,[255],[0,255])  

        red_histh_list = [val[0] for val in red_histh]
        red_hists_list = [val[0] for val in red_hists]
        red_histv_list = [val[0] for val in red_histv]

        green_histh_list = [val[0] for val in green_histh]
        green_hists_list = [val[0] for val in green_hists]
        green_histv_list = [val[0] for val in green_histv]

        blue_histh_list = [val[0] for val in blue_histh]
        blue_hists_list = [val[0] for val in blue_hists]
        blue_histv_list = [val[0] for val in blue_histv]

        yellow_histh_list = [val[0] for val in yellow_histh]
        yellow_hists_list = [val[0] for val in yellow_hists]
        yellow_histv_list = [val[0] for val in yellow_histv]

        orange_histh_list = [val[0] for val in orange_histh]
        orange_hists_list = [val[0] for val in orange_hists]
        orange_histv_list = [val[0] for val in orange_histv]

        pink_histh_list = [val[0] for val in pink_histh]
        pink_hists_list = [val[0] for val in pink_hists]
        pink_histv_list = [val[0] for val in pink_histv]
        
        
        # if(max(red_histh_list) > 200):
        #     max_h = red_histh_list.index(max(red_histh_list))
        #     red_lower[0] = max_h - 5
        #     red_upper[0] = max_h + 5
        if(max(red_hists_list) > 40):
            max_s = red_hists_list.index(max(red_hists_list))
            red_lower[1] = max_s - 40
            red_upper[1] = max_s + 40
        if(max(red_histv_list) > 30):
        ...
        if(max(green_histv_list) > 30):
            max_v = green_histv_list.index(max(green_histv_list))
        ...
        if(max(yellow_histh_list) > 200 and max(yellow_histh_list) < 500):
            max_h = yellow_histh_list.index(max(yellow_histh_list))
            yellow_lower[0] = max_h - 13
            yellow_upper[0] = max_h + 13
        if(max(yellow_hists_list) > 40):
            max_s = yellow_hists_list.index(max(yellow_hists_list))
            yellow_lower[1] = max_s - 20
            yellow_upper[1] = max_s + 20
        if(max(yellow_histv_list) > 30):
            max_v = yellow_histv_list.index(max(yellow_histv_list))
            yellow_lower[2] = max_v - 60
            yellow_upper[2] = max_v + 60


        # if(max(orange_histh_list) > 200):
        #     max_h = orange_histh_list.index(max(orange_histh_list))
        #     orange_lower[0] = max_h - 5
        #     orange_upper[0] = max_h + 5
        if(max(orange_hists_list) > 40):
            max_s = orange_hists_list.index(max(orange_hists_list))
            orange_lower[1] = max_s - 40
            orange_upper[1] = max_s + 40
        if(max(orange_histv_list) > 30):
            max_v = orange_histv_list.index(max(orange_histv_list))
            orange_lower[2] = max_v - 40
            orange_upper[2] = max_v + 40

        # if(max(pink_histh_list) > 200):
        #     max_h = pink_histh_list.index(max(pink_histh_list))
        #     pink_lower[0] = max_h - 5
        #     pink_upper[0] = max_h + 5
        if(max(pink_hists_list) > 40):
            max_s = pink_hists_list.index(max(pink_hists_list))
            pink_lower[1] = max_s - 40
            pink_upper[1] = max_s + 40
        if(max(pink_histv_list) > 30):
            max_v = pink_histv_list.index(max(pink_histv_list))
