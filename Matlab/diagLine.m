...
xc1d = tspan * 0.1;%0.3 * cos(tspan);
yc1d = tspan * 0.1;%0.3 * sin(tspan);
theta1d = ones(1, length(tspan))*pi/4;%unwrap(atan2(cos(tspan), -0.8 * sin(tspan)));
...
for q = 1 : length(tspan)
    ...
    v1(q) = (x13(q) * Xi11(q + 1)) + k4 * Xi12(q + 1);
    w1(q) = Xi11(q + 1);
