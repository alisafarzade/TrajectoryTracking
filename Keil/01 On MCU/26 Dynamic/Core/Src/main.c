...
float kpR  = 0.5  , kpL = 0.5;
float ki_R = 1.0 , ki_L = 1.0;
float kd_R = 0.05, kd_L = 0.05;
...
float k1 = 12.0,k2 = 12.0,k3 = 3.0, k4 = 1.0, k5 = 1.0, k41 = 2, k42 = 2;
float V, w;
...
int neurons = 30;//2160;
float centers[30][6], phi_vec[30];
float Xi_dot_Virtual[2], Xi_Virtual[2], input_Vector[6];
float Xi1_dot, Xi2_dot, x3_dot;
float last_Xi1, last_Xi2, last_x3;
...
float ycd(float time){
	return 0.45 * sin(time*speed);// + 1.7;
}
float thetad(float time){
	float theta = atan2(cos(time*speed), -sin(time*speed));
...
void createCenters(){
	...
	float A3[] = { 0.0,  1.0, 2.0};
	float A4[] = {-1.5, -0.5, 0.5, 1.5};
	float A5[] = {-1.0,  0.0, 1.0};
	float A6[] = { 0.0,  1.0, 2.0};
	int cnt = 0;
	for(int i=0; i<4; i++){
		for(int j=0; j<5; j++){
			for(int k=0; k<3; k++){
...
void update_inputVector(float in1, float in2, float in3, float in4, float in5, float in6){
	input_Vector[0] = in1;
	input_Vector[1] = in2;
	input_Vector[2] = in3;
	input_Vector[3] = in4;
...
float normal;
void update_phi_vec(int index){
	float temp, sum1 = 0;
	for(int i=0; i<6; i++){
		temp = pow(input_Vector[i] - centers[index][i], 2);
		sum1 = sum1 + temp;
	}
	normal = sqrt((double)sum1);
	phi_vec[index] = exp(-pow(normal, 2)/1.96);
}
...
//		
		...
		*/
		...
		if(HAL_GetTick() - theta_pid_last_time > 50){
		...
		if(derivationFlag){
			delta_time = 0.2;//((TIM4_PERIOD+1)/1000);
			Xi1_dot = (Xi1 - last_Xi1)/delta_time;
			Xi2_dot = (Xi2 - last_Xi2)/delta_time;
			x3_dot  = (_x3 - last_x3 )/delta_time;
			last_Xi1 = Xi1;
			last_Xi2 = Xi2;
			last_x3 = _x3;
			derivationFlag = 0;
		...
		Xi_Virtual[0] = Xi1;
		Xi_Virtual[1] = Xi2;
		Xi_dot_Virtual[0] = Xi1_dot;
		Xi_dot_Virtual[1] = Xi2_dot;
		
		update_inputVector(Xi1_dot, Xi2_dot, Xi1, Xi2, _x3, x3_dot);
...
//		rpmLeftD = speedAngle + addSpeed;
		...
		/////////////////////////////////////////////////// PID Farhan
		if(HAL_GetTick() - pid_last_time > 20){
			...
			//if(rpmRight != rpmRightD) {
				...
				difrentialError_r = (errorRight - lastErrorRight)/0.02;
			//}else{
			//	integralError_r   = 0;
			//	difrentialError_r = 0;
			//}
			//if(rpmLeft != rpmLeftD) {
				...
				difrentialError_l = (errorLeft  - lastErrorLeftt)/0.02;
			//}else{
			//	integralError_l   = 0;
			//	difrentialError_l = 0;
			//}
