...
float wd(float time){
	...
	float wd_f = atan2(_ycd - y, _xcd - x) * 0.3;
	//if(wd_f < 0) wd_f+=2*pi;
	return wd_f;
}
...
Xi1 = _wd + (k3 * (_thetad - angle)); // 0
z1 = _x3d - _x3; // 0
A = pow(le1,2) - pow(z1,2);
alpha = _x2d + (A * k1 * z1 * _wd); // 5
z2 = alpha - _x2; // 0
B = pow(le2,2) - pow(z2,2);

if(derivationFlag){
	alphadot = (alpha - lastalpha)/0.2; // 0
	derivationFlag = 0;
	lastalpha = alpha;
...
Xi2 = alphadot + (B * k2 * z2 * pow(_wd,2)) + ((B / A) * z1 * _wd); // 0
V = _x3 * Xi1 + Xi2;
w = Xi1;
