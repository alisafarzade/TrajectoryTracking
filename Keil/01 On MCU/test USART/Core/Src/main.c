...
int  contL = 0, contR = 0, currentL, currentR;
...
//	HAL_ADC_Start_DMA(&hadc1, &AD_RES, 1);
  ...
  while (1)
  {
		...
		sumL += (float) readValueL * 3.3 * 1.05 / 4095 ;
		contL++;
		if(contL >= 50){
			rawVoltageL = (float)sumL / 50;
			contL = 0;
			sumL = 0;
			currentL = rawVoltageL * 49;
		}
		...
		sumR += (float) readValueR * 3.3 * 1.05 / 4095 ;
		contR++;
		if(contR >= 50){
			rawVoltageR = (float)sumR / 50;
			contR = 0;
			sumR = 0;
			currentR = rawVoltageR * 54;
		}
...
//		uint8_t Rx_data[10];
//		Response = HAL_UART_Receive(&huart1, Rx_data, 3, 50);
////		uint8_t data[] = "Hello!";
//		if(Response == HAL_OK){
//		
//					HAL_UART_Transmit(&huart1, Rx_data, sizeof(Rx_data)-1, 10);
