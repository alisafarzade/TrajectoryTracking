...
float k1 = 1.0,k2 = 1.0,k3 = 0.5, k4 = 1.0, k5 = 1.0, k41 = 2, k42 = 2;
float V, w;
...
float xcd(float time){
	return 0.60 * sin(time*speed);
}
float ycd(float time){
	return 0.30 * sin(2*time*speed);
}
float thetad(float time){
	float theta = atan2(0.60*cos(2*time*speed), 0.60*cos(time*speed));
...
float wd(float time){
	// Shape8
	return -(speed * (2*cos(speed*time)*sin(2*speed*time) - sin(speed*time)*cos(2*speed*time))) / (cos2(2*speed*time)+cos2(speed*time));
